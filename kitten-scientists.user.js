...
window.run = function() {
	...
	const i18nData = {
		'zh': {
			...
			'summary.auto.moonBase': '男德素存到80%，小猫才会有力气造月球基地',
	...
	Engine.prototype = {
			...
			for (let i = village.jobs.length - 1; i >= 0; i--) {
				...
				if (name === 'woodcutter') {
					if (game.getEffect('rankLeaderBonusConversion')) {
						let leader = village.leader;
						if (leader && leader.job === 'woodcutter' && revolution < 0.4 && leader.rank > 2) {
				...
				if (name === 'scholar') {
					...
					if (val > 2 && !resMap['coal'].value) {maxKS *= 0.6;}
					if (!atheism && !resMap['faith'].value && resMap['science'].value > 2e4) {maxKS = 0;}
					...
					if (resPercent('science') > moreScholar && val || !woodcutter || (woodcutter < 2 && !resMap['paragon'].value)) {
						...
						maxKS = Math.max(maxKS, 24 - 9 * (moreScholar < 0.5) - 3 * (revolution < 0.4));
		...
		worship: function () {
			...
			if (booleanForPraise || forceStep || fPraise) {
				...
				if (msgPraise && (Date.now() > option.autoPraise.time + 4e5 || !activitySummary.other['praise.msg']) && solarRatio) {
		...
		upgrade: function () {
			...
			if (upgrades.techs.enabled && game["libraryTab"].visible) {
				...
				let acoustics = !game.science.get('acoustics').researched && resMap['science'].value > 6e4 && Religion.transcendenceTier && !titanium;
					...
					if (upgrades.techs.limited) {
						...
						switch (name) {
							...
							case 'oilProcessing':
								...
								break;
							case 'drama': {
								if (craftManager.getTickVal(parchment, true) < 3 && parchment.value < 15e3 && !resMap['eludium'].value) {
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					if (game.bld.getBuildingExt('steamworks').meta.on < 1 + 5 * priceRatio) {
					...
					// 缺电过滤碳封存
					if (game.resPool.energyProd - game.resPool.energyCons < 15 + 25 * (Production < 0)) {
					...
					if (game.bld.getBuildingExt('factory').meta.val < 3) {
						noop = noop.concat(['factoryLogistics', 'refrigeration']);
					...
					if (game.ironWill) {
						...
						// 农民工具升级过滤
						if (game.village.jobs[0].value > 3 && Production > 2) {
			...
			if (upgrades.policies.enabled && game["libraryTab"].visible) {
				...
				(function () {
					...
					const toResearch = [];
					let ratio = (Science.get('astronomy').researched || !Science.get('philosophy').researched || game.getEffect('priceRatio') > -0.03) ? 1 : 3;
			...
			if (upgrades.buildings.enabled) {
				...
				if (aqueductMeta.stage === 0) {
					if (aqueductMeta.stages[1].stageUnlocked) {
						if (pastureMeta.stage === 1) {
							...
							let catnip = catnipTick > 0 || (catnipTick < 0 && resMap['catnip'].value + (4e3 + 6e3 * (revolutionRatio < 8)) * catnipTick > 0);
				...
				if (libraryMeta.stage === 0) {
					if (libraryMeta.stages[1].stageUnlocked) {
						...
						if (ratio && items.dataCenter.enabled && resMap['eludium'].value) {
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (theology) {
					...
					if (machinery && resPercent('wood') < 0.9 && !game.ironWill) {library.max = 45;}
					if (Production && winterTick > 4 && hutVal < 4 && hutVal > 1) {
						if (Religion.transcendenceTier > 10 && !machinery) {
							if (winterTick > 5) {pasture.max = 0;}
							items['barn'].max = 0;
						}
						if (!science.get('construction').researched) {
							if (hutVal === 2) {
								items['library'].max = 13 + Production;
								items['mine'].max = 1;
								pasture.max = 13;
							}
							if (hutVal === 3) {
								library.max = 30 - 2 *Production;
								items['academy'].max = 5;
							}
						}
					}
				...
				if (!game.challenges.isActive("pacifism")) {
					...
					if (!magnetoMeta.on && !game.ironWill) {
				...
				if (hasLeader &&!game.getEffect('unobtainiumPerTickSpace') && resMap['faith'].maxValue < 8000) {
					temple.max = Math.max(21 + 10 * orbitalGeodesy, 10 * (1 + revolutionRatio));
				}
				// 无、低太阳革命
				if (revolutionRatio < 0.05) {
					Brewery.max = 22;
					...
					if (smelterVal) {
						if (resMap['wood'].perTickCached * 2 < smelterVal || resMap['minerals'].perTickCached < smelterVal + 1 - 3 * theology - Production) {
							smelter.max = 0;
							msgSummary('smelter');
						} else {
				...
				if (geodesy) {
					...
					if (religionRU && revolutionRatio) {
						...
						if (!game.ironWill) {val = Math.max(val, 12);}
						tradepost.max = val;
						msgSummary('religion');
					}
					// 测地学 交易所
					if (!revolutionRatio) {
						tradepost.max = 35 - 25 * (Production > 4) - 5 * hasLeader;
					}
				}
				// 太阳革命前不造交易所和神殿
				...
				if (!theology && science.get('philosophy').researched && goldTri < 0.96 && !atheism) {
					...
					tradepost.max = 20 - 2 * Production - 4 * hasLeader - 0.5 * (resMap['kittens'].value > 54);
					msgSummary('tradepost');
				}
				...
				if (orbitalGeodesy) {
					if (!spaceManufacturing || !sattelite) {
						let Max = Math.min(47, 6 * revolutionRatio + 15 + 2 * Production + 4 * game.workshop.get('unobtainiumDrill').researched);
					...
					if (!game.workshop.get('oxidation').researched) {calciner.max = 5 * (1 + revolutionRatio) * (1 + 0.2 * Production);}
					items['warehouse'].max = 54 - 200 * priceRatio + 50 * vitruvianFeline;
				}

				let calcinerMeta = game.bld.getBuildingExt('calciner').meta;
				let oilTick = resMap['oil'].perTickCached - 0.24 * calcinerMeta.val - 0.05 * magnetoMeta.val;
				if (oilTick < 0.5) {
				...
				if (!geodesy && !orbitalGeodesy) {
					if (!game.ironWill && mansion.max) {
						mansion.max = Math.max(16, 5 * (1 + revolutionRatio) * (1 + Production));
			...
			for (let name in builds) {
				...
				if (meta.almostLimited && (!geodesy || resMap['kittens'].maxValue < 96) && hasLeader) {build.enabled = false;}
		...
		space: function () {
			...
			// 自动项目
			{
				...
				if (!trigger) {
					...
					// 月球前哨
					if ((game.getEffect('unobtainiumPerTickSpace') && uranium < 0.7) || resPercent('unobtainium') === 1) {builds['moonOutpost'].max = 0;}
					// 月球基地
					...
					// 星球裂解
					if (uranium > 8 && starchartVal < 1e6) {builds['planetCracker'].max = 0;}
					// 流体切割
		...
		holdFestival: function () {
			...
			if (game.resPool.hasRes(price)) {
				...
				if (game.workshop.get('nanosuits').researched) {options.auto.trade.festival = true;}
			}
		},
		...
		trade: function () {
			...
			let transcendence = (Religion.getRU("transcendence").on || !Faith.transcendence.enabled || !game.space.getBuilding('moonOutpost').val);
			...
			let Moon = tier || Religion.transcendenceTier > 9 || game.village.sim.kittens.length < 130;
			...
			isLimited = (isLimited && !isGoldTrigger) ? 0.3 + 0.05 * vitruvian + 0.1 *!solarRevolution + 0.05 * game.workshop.get('geodesy').researched : 1;
		...
		miscOptions: function () {
			...
			if (optItem.buildEmbassies.enabled && !!game.diplomacy.races[0].embassyPrices) {
				...
				let highCulture = cultureTri > Math.max(0.96, options.auto.craft.trigger);
				let solarFactor = Math.sqrt(Religion.getSolarRevolutionRatio() + 1);
				let cultureForce = culture.value > 5e3 + 5e3 * (tt > 5) + 5e3 * Math.sqrt(2 * Religion.getSolarRevolutionRatio());
	...
	UpgradeManager.prototype = {
		...
		getValue: function (name, upgrade) {
			...
			if (name === 'titanium' && upgrade !== 'rotaryKiln') {
				...
				if (cache === 'orbitalGeodesy' && cache !== upgrade) {
					if (upgrade !== 'pumpjack') {
						stock += 1e3;
					} else if (resMap['oil'].value > 35e3) {
						stock += 1e3;
					}
				}
				if (cache === 'concreteHuts' && cache !== upgrade) {stock += 3e3;}
			}
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'temple':
					count = 1;
					if (resPercent('gold') < 0.7 && Workshop.get('miningDrill').researched && orbitalGeodesy) {halfCount = true;}
					break;
				case 'mint':
					count = 1;
					if (resPercent('gold') < 0.95 && game.getEffect('manpowerPerTickCon') < -2) {halfCount = true;}
					break;
				case 'field':
				...
				case 'aqueduct': {
					if ((this.crafts.getPotentialCatnip(false) < 4 * game.village.happiness && resPercent('catnip') < 0.6) || game.challenges.isActive('winterIsComing')) {
						...
						if (resMap['slab'].value < 4e5 * (1 + revolution)) {
				...
				case 'oilWell':
					if (id === 'oilWell') {
						...
						if (!spaceManufacturing) {
							...
							if (cal > 1) {
								if ((!orbitalGeodesy && Val < 3e4) || (cal > 2 && maxVal < 5e4)) {return count;}
							}
							if (meta < 199) {
								if (maxVal > 90e3) {
									count = Math.floor(count * 0.5);
								} else if (maxVal > 20e3 && cal < 2) {
									halfCount = true;
								}
							}
						}
				...
				case 'harbor':
					if (id === 'harbor') {
						...
						if (harborVal < 3 && revolution < 1) {return 1;}
						if (harborVal < 8) {
							msgSummary('harbor');
							halfCount = true;
				...
				case 'logHouse':{
					...
					if (am.val < 5 && !am.stage && am.unlocked && game.village.happiness < 3 && game.village.maxKittens > 26) {
						msgSummary('logHouse');
						return 0;
					}
					break;
				...
				case 'factory': {
					...
					if (spaceManufacturing) {
						...
						if (!TitaniumCap) {
							if (faVal) {
								if (geodesy && Workshop.get("nuclearSmelters").researched && faVal < 12) {return count;}
								count = Math.floor((0.4 + 0.3 * !vitruvianFeline) * count);
								halfCount = true;
								msgSummary('factory');
							} else {
								count = 1;
			...
			if ((id === 'hut' || id === 'logHouse') && count) {
				...
				let noFarmer = Catnip.value < 5e3 + happy * 100 && Catnip.perTickCached < 1.7 * happy * (1 + 2 * anarchy) && (!game.science.get('agriculture').researched || anarchy);
				let forceCat = catnipTick < 1 && Catnip.value + catnipTick * 1000 * (resPercent('catnip') < 0.1) - 1000;
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'ship' && limited) {
				...
				if (force) {
					...
					if (geodesy) {
						forceShipVal = 243;
						if (Religion.faithRatio && resMap['science'].maxValue > 11e4) {
							if (tt < 6 && geodesy) {forceShipVal = -103 * tt + 750 - 250 * !tt;}
						}
					}
					if (shipValue < forceShipVal && ratio > 3) {
						autoMax = 1;
					} else {
						force = false;
					}
			...
			if (name === 'beam' || name === 'slab') {
				...
				if (resPercent(i) < 2) {
					...
					if (!value && resMap['slab'].value) {force = true;}
					if (name === 'slab') {
						if (!resMap['concrate'].unlocked && game.science.get('mechanization').researched && resMap['steel'].value > 25) {
							let number = Math.ceil((2500 - resValue) / ratio);
							if (number > 0 && resMap['minerals'].value > number * 250) {
								autoMax = number;
								force = true;
							}
						}
						if (geodesy && !game.ironWill && resMap['kittens'].maxValue < 135 && workshop.get("concreteHuts").researched) {
			...
			if (name === 'plate' && limited) {
				...
				if (react || (resPercent('iron') > 0.89 && resMap['titanium'].maxValue < 3500 * reactRatio)) {
					let number = Math.ceil((5000 * reactRatio - resValue) / ratio);
					if (number > Math.max(autoMax)) {
						autoMax = number;
						force = true;
					}
				}
			...
			if (name === 'manuscript' && limited) {
				...
				for (i = 16; i < indexMax; i++) {
					...
					if (!meta.researched || cacheManuscript > 0) {
						...
						if (autoMax >= 1) {
							let parchmentVal = this.getValueAvailable('parchment', true);
							if (parchmentVal > autoMax * (25 - 5 * tradition) && resMap['culture'].value > (400 - 100 * tradition) * autoMax) {
			...
			if (name === 'compedium' && limited && navigation) {
				...
				// 物理学 => 强制到声学
				if (i === 20 && value < 60 && Religion.transcendenceTier) {
					force = true;
					autoMax = 1;
				}
				// 冶金
				if (!Science.get('metalurgy').researched && cultureTri > 0.98 && resValue < 100 && scienceTri > 0.98) {
					...
					if (maxVal > 85e3 || (!Science.get('electricity').researched && resValue < 85)) {
						force = true;
					}
			...
			// 混凝土
			if (name ==='concrate' && !resMap['concrate'].unlocked) {
				force = true;
				autoMax = 1;
			}
			// 羊皮纸
			if (name === 'parchment') {
				...
				if (value < 5 && ratio > 1.5 && resMap['minerals'].value > 600) {
					force = true;
					autoMax = 1;
				}
				if (!navigation) {
					if (resValue > 200) {
						limited = true;
						if (resValue > 400) {autoMax = 1;}
						if (resValue > 500) {autoMax = 0;}
			...
			if (name === 'alloy' && limited) {
				let forceAlloy = (name, price) => {
					...
					if (amt > 1 && resMap['steel'].value > amt * 75 && resMap['titanium'].value > amt * 10 && !Craft.oxidation) {
						...
						if (!ti || (ti && resMap['titanium'].value - price > ti.stock)) {
							amount = amt;
							force = true;
							if (Religion.getSolarRevolutionRatio() < 7 || price > 25) {activity(i18n('craft.forceSteel', [workshopMeta.label]));}
						}
					}
				};
				...
				if (game.getEffect('hunterRatio')) {forceAlloy('alloySaw', 250);}
				if (resMap['oil'].value > 5e4 && resMap['uranium'].value > 250) {forceAlloy('photolithography', 1250);}
			}
		...
		getTickVal: function (res, preTrade) {
			...
			if (!preTrade) {
				...
				if (name === 'unobtainium' && Religion.getSolarRevolutionRatio() > 9) {
					ratio = 0.4;
					if (game.calendar.cycle === 5) {ratio = 0.15;}
				}
			...
			['wood','minerals','iron', 'alloy', 'titanium', 'steel', 'gear', 'beam', 'blueprint'].forEach((name) => {
				...
				switch (name) {
					case 'wood': {
						...
						if (kittens < 130 && workshop.get('concreteHuts').researched && !iw && !game.getEffect("priceRatio") && resPercent(name) < 1) {stock += 11e5;}
						break;
					}
					case 'minerals': {
						if (kittens < 130 && workshop.get('concreteHuts').researched && !iw && !game.getEffect("priceRatio") && resPercent(name) < 1) {stock += 13e5;}
						break;
					}
					case 'iron': {
						let priceRatio = game.getEffect("priceRatio");
						let lumberMill = game.bld.getBuildingExt('lumberMill').meta.val > 19 - 40 * priceRatio;
						let a = resMap['gold'].value < 50 + 25 * (priceRatio > -0.02) && lumberMill;
						if (game.bld.getBuildingExt('temple').meta.on > 2 || resMap['faith'].maxValue > 749 || a || resMap['plate'].value > 18) {
							...
							let crossbow = this.getUnResearched('crossbow') && lumberMill && maxVal > 1500;
							let ironwood = this.getUnResearched('ironwood') && val > 700 && maxVal > 3000
					...
					case 'steel': {
						...
						if (upg.cache === 'concreteHuts') {stock += 750;}
					...
					case 'titanium': {
						...
						if (geodesy && resMap['starchart'].value > 420 && (cal || anarchy)) {stock += 250;}
						...
						if (geodesyResearched) {
							...
							if (miningDrill && resMap['titanium'].value > 675) {
								let upg = options.auto.upgrade.items.upgrades;
								if (!upg.cache) {upg.cache = 'concreteHuts';}
							}
						}
						// 混凝土小屋
						if (this.getUnResearched('concreteHuts') && !iw && resMap['concrate'].value > 45 && resMap['science'].maxValue > 12e4 && resMap['uranium'].value < 250) {
							if ((geodesyResearched && resMap['titanium'].value > 750) || (geodesy && resMap['titanium'].value > 1e3)) {
								stock += 3e3;
								let upg = options.auto.upgrade.items.upgrades;
								if (!upg.cache) {upg.cache = 'concreteHuts';}
							}
					...
					case 'gear':
						...
						// 燃料喷射器 待定
						if (this.getUnResearched('fuelInjectors') && resMap['oil'].value > 2e4 && game.getEffect('calcinerRatio') < 1 && Religion.getSolarRevolutionRatio() < 5) {stock += 250;}
						// 抽油机
						...
						// 石油加工
						if (this.getUnResearched('oilRefinery') && resMap['titanium'].value > 1250 && resMap['science'].maxValue > 125e3) {stock += 500;}
						break;
					case 'beam':
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'slab': {
					...
					limRat = (scaffold || start || (reactor.val && resVal < 2500)) ? limRat : 5e-3;
					limRat = (scaffold < 500 || (scaffold > 500 && resVal < 615)) ? limRat : 0.1;
				...
				case 'ship': {
					...
					limRat = (game.workshop.get('geodesy').researched && shipValue < 3 * shipLimit && solar < 4) ? 0.7 : 0.4;
				...
				case 'scaffold': {
					...
					limRat = (navigation && resMap['iron'].value > 750 || game.science.get('chemistry').researched) ? limRat : 0;
					let lowScience = (val < 880 && resMap['science'].maxValue < 110e3 + 9e3 * (resMap['culture'].maxValue > 15e3) && shipValue > 243);
					limRat = ((val < 100 + 100 * solar && navigation && resMap['iron'].value > 750) || lowScience) ? 1 : limRat;
					break;
				}
				case 'tanker':
					limRat = 0.1;
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			if (name === 'griffins') {
				...
				if (game.challenges.isActive("blackSky") && titanium.value < 500 && Religion.getRU("basilica").on && season === 2) {doTrade = true;}
				// 缺铁贸易狮鹫
			...
			if (name === 'zebras') {
				...
				if (season === 1) {
					if (Workshop.get("geodesy").researched && resPercent('titanium') < 0.5) {doTrade = true;}
				...
				if (Workshop.get("geodesy").researched) {
					if ((game.getEffect('calcinerRatio') && titanium.value < 5e3) || !Workshop.get("concreteHuts").researched || Workshop.get("miningDrill").researched) {
						doTrade = true;
					}
				}
	...
	i18nLang.messages['policy.socialism.desc'] = '';
