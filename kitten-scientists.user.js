...
window.run = function() {
			...
			if (upgrades.buildings.enabled) {
				...
				if (libraryMeta.stage === 0) {
					if (libraryMeta.stages[1].stageUnlocked) {
						...
						let ratio = resMap['compedium'].value * 3 > scienceBldMax / (1 + game.bld.get('biolab').val * 0.01)
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (theology) {
					...
					if (machinery && resPercent('wood') < 0.9 && !game.ironWill) {items['library'].max = 45;}
				}
				...
				if (!revolutionRatio) {
					...
					temple.max = 35 - 28 * !geodesy;
					let smelterVal = game.bld.getBuildingExt('smelter').meta.val;
					if (smelterVal) {
						if (resMap['wood'].perTickCached * 2 < smelterVal || resMap['minerals'].perTickCached < smelterVal - 3 * hasLeader) {
							smelter.max = 0;
							msgSummary('smelter');
						}
					}
				...
				if (geodesy) {
					...
					// 测地学 交易所
					if (!revolutionRatio) {
						tradepost.max = 35 - 25 * (Production > 4) - 5 * revolutionRatio - 5 * hasLeader;
					}
				}
				// 太阳革命前不造交易所和神殿
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'chapel':
					...
					break;
				case 'aqueduct': {
					if ((this.crafts.getPotentialCatnip(false) < 2 && resPercent('catnip') < 0.5) || game.challenges.isActive('winterIsComing')) {
						break;
				...
				case 'oilWell':
					if (id === 'oilWell') {
						...
						if (oil.maxValue > 9e3 && Val > 3e4 && meta < 199) {count = Math.floor(count * 0.5);}
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'plate' && limited) {
				...
				if (react || (resPercent('iron') > 0.89 && resMap['titanium'].maxValue < 3500 * reactRatio)) {
					autoMax = Math.ceil((5000 * reactRatio - resValue) / ratio);
					force = true;
				}
				if (resValue < 150) {
					if (resMap['faith'].maxValue < 750 && resMap['gold'].value > 30 && value < templePriceRatio * 15 &&
						(ratio > 2.8 || resMap['iron'].value / 125 * ratio > 10 || game.village.leader.rank > 3)) {
						force = true;
						autoMax = 1;
					}
			...
			if (name === 'compedium' && limited && navigation) {
				...
				// 物理学 => 强制到声学
				if (i === 20 && value < 60 && game.religion.transcendenceTier) {
					force = true;
					autoMax = 1;
				}
				// 冶金
			...
			if (name === 'steel' && limited) {
				...
				let ironCoalRatio = Math.max(1 * !navigation, 0.3 * Math.min(1, resMap['coal'].perTickCached / resMap['iron'].perTickCached));
				if (resMap['plate'].value < 1.25 * ironCoalRatio * resMap['steel'].value && !Craft.oxidation && Craft.items['plate'].enabled && steelValAva) {
					amount = 0;
				}
		...
		getTickVal: function (res, preTrade) {
			...
			if (!preTrade) {
				let ratio = 1;
				if (name === 'unobtainium' && game.religion.getSolarRevolutionRatio() > 9) {
					ratio = 0.4;
		...
		getAutoStocks: function () {
			...
			['wood','iron', 'alloy', 'steel', 'titanium', 'gear', 'beam', 'blueprint'].forEach((name) => {
				...
				switch (name) {
					...
					case 'iron': {
						let priceRatio = game.getEffect("priceRatio");
						let lumberMill = game.bld.get('lumberMill').val > 19 - 40 * priceRatio;
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'scaffold': {
					...
					limRat = (game.science.get('navigation').unlocked && resMap['iron'].value > 750 || game.science.get('chemistry').researched) ? limRat : 0;
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			if (name === 'nagas') {
				...
				if (Val > 250 * (1 + solar) * production * (1 + solar) && titaniumTri < 0.8) {prof = false;}
			}
