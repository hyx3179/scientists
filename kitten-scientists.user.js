...
window.run = function() {
	...
	const i18nData = {
		'zh': {
			...
			'act.timeCrystalSacrifice' : '小猫精炼了 {0} 时间水晶，获得了 {1} 古神的遗物',
			...
			'summary.timeCrystal': '一共精炼 {0} 时间水晶',
		...
		auto: {
			...
			faith: {
				...
				items: {
					...
					blackRadiance:      {require: false,         enabled: false, max:20, variant: 'c', checkForReset: true, triggerForReset: -1},
	...
	Engine.prototype = {
		...
		timeCtrl: function () {
			...
			if (optItem.timeSkip.enabled && game.workshop.get('chronoforge').researched) {
				...
				if (currentCycle === 5 && optItem.timeSkip[currentCycle] && prestige) {
					let Nummon = game["nummon"];
					if (game["nummon"]) {
						// 概览 算好的当前周期收入
						let unobtainiumTick = Nummon['getTradeTC']();
				...
				if (name === 'geologist') {
					...
					if (!geodesy) {
						if (resMap['iron'].perTickCached < 20) {
							maxKS = Math.round(maxKS * (0.25 - 2 * priceRatio) + tt);
		...
		worship: function () {
			...
			if (option.tcRefine.enabled) {
				if (game.getEffect('relicRefineRatio') > 35 && zigguratOn) {
					...
					if (shatterTCGain > 0.15 && game.getEffect('heatPerTick') > 0.64 && timeCrystalVal > 5e4 && game.calendar.cycle !== 5 && game.karmaKittens > 3e4) {
						if (shatterTCGain > 0.19 && relicVal < 2e7) {
							doTransform = 3;
							if (shatterTCGain > 0.29) {
								doTransform = 20;
							}
						}
						if (game.getEffect('solarRevolutionLimit') < 70 && relicVal < 1.8e6 && game.getEffect('blsLimit') > 24) {
							doTransform = 20;
						}
					}
					if (doTransform) {
						...
						storeForSummary('timeCrystal', doTransform * 25);
			...
			if (Math.min(0.999, Math.max(0.98, PraiseSubTrigger)) <= rate || doAdoreAfterTimeSkip) {
				...
				if (option.transcend.enabled && transcendenceReached) {
					...
					while (TranscendTimes) {
						...
						if (shatterTCGain > 0.07 && game.calendar.trueYear < 1400) {obeliskRatio = 1;}
			...
			if (booleanForPraise || forceStep || fPraise) {
				...
				let times = (transcendenceTier < 4) ? 1e5 : 2e6;
			...
			if (resMap['timeCrystal'].value < 1e10) {
				...
				if (shatterTCGain > 0.04) {
					...
					if (shatterTCGain > 0.07) {
						if (game.getEffect("heatEfficiency")) {
							number = Math.round(122 * shatterTCGain + 11);
						} else {
		...
		upgrade: function () {
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					// 过滤钛升级
					if (game.globalEffectsCached['titaniumPerTickAutoprod'] < 0.02 && resMap['ship'].value < 60) {
			...
			if (typeof builds != 'object') {
				...
				if (game.challenges.isActive("anarchy")) {
					...
					if (!game.getEffect('aiCoreProductivness') && vitruvianFeline) {
						if (game.getEffect('beaconRelicsPerDay')) {
							let gf = game.getEffect('gflopsConsumption');
							if (resMap['burnedParagon'].value < 2e4 || !gf) {
								items['aiCore'].max = (gf) ? 4 + 21 * (game.calendar.year > 3e3) : 3;
							}
						} else {
							items['aiCore'].max = 0;
						}
						items['chronosphere'].max = 10 + 10 * (resMap['paragon'].value > 2e3 || game.calendar.year > 200) + 20 * !revolutionRatio + 20 * (Production > 4);
					}
		...
		space: function () {
			...
			// 自动项目
			{
				...
				if (trigger) {
					...
					let chamberMeta = game.space.getBuilding("containmentChamber");
					let containmentChamber = chamberMeta.val;
					...
					// 月球前哨
					if (uranium < 1 + (1 + 8 * spaceManufacturing) * (Production < 1.4) || resPercent('unobtainium') >= 1) {
					...
					// 星球裂解
					if (uranium > 3 - 2 * vitruvianFeline && starchartVal < 1e6) {
					...
					itemChamber.max = chamberMeta.on + 1;
					...
					if (game.getEffect('beaconRelicsPerDay')) {
						...
						if (Year < 1e3) {
							...
							if (!years.on && years.active) {
								...
								let tc = options.auto.resources.timeCrystal;
								if (tc && tc.stock) {
									tc.stock = 0;
								}
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'aiCore':
					count = 1;
					if (game.getEffect('gflopsPerTickBase') > 0.6 && resMap['burnedParagon'].value > 1e5 && game.getEffect('globalResourceRatio') < 3.5) {
				...
				case 'harbor':
					if (id === 'harbor') {
						...
						} else if (reactorVal < 30 + 9 * vitruvianFeline) {
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'alloy' && limited) {
				...
				if (!game.getEffect('unobtainiumPerTickSpace') && options.auto.upgrade.items.upgrades.cache !== 'geodesy') {
					...
					if (resMap['wood'].perTickCached && game.religion.faith < 1e6) {
		...
		getTickVal: function (res, preTrade) {
			...
			if (name === 'timeCrystal') {
				let paragon = resMap['burnedParagon'].value;
				if (paragon < 1e5 + 2e6 * (game.getEffect("shatterTCGain") > 0)) {
					...
					if (game.getEffect('antimatterProduction') > 22 && resPercent('unobtainium') < 0.6) {prod *= 30 - 25 * (paragon > 1e5);}
				}
			}
			if (res.craftable) {
		...
		getAutoStocks: function () {
			...
			['wood','minerals','iron', 'alloy', 'titanium', 'steel', 'gear', 'beam', 'blueprint'].forEach((name) => {
				...
				switch (name) {
					...
					case 'minerals': {
						if (Auto.build.enabled < 130 && Workshop.get('concreteHuts').researched && !iw && !priceRatio && resPercent(name) < 0.99 && game.getEffect('craftRatio') >=3) {
					...
					case 'steel': {
						...
						// 钛金粮仓
						if (this.getUnResearched('titaniumBarns') && resMap['ship'].value > 70 - 200 * priceRatio && titaniumVal > 25) {
					...
					case 'titanium': {
						...
						if (!iw && geodesy && resMap['starchart'].value > 350 - 50 * !calcinerRatio - 25 * vitruvianFeline) {
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'manuscript': {
					...
					if (reactorVal && resPercent('culture') > 0.5 + 0.4 * (solar > 70)) {
						let brewery = Bld.getBuildingExt('brewery').meta;
						if (resMap['culture'].maxValue > 750 * Math.pow(brewery.priceRatio + priceRatio, brewery.val) ) {
							let chapel = Bld.getBuildingExt('chapel').meta;
							chapel = Math.pow(chapel.priceRatio + priceRatio, chapel.val);
							if (resMap['parchment'].value > Math.max(1e6, 260 * chapel)) {limRat = 1;}
						}
					}
					break;
				}
	...
	TradeManager.prototype = {
		...
		trade: function (name, amount) {
			...
			let button = this.getTradeButton(name);
			if (!button) {
				game['diplomacyTab'].render();
				button = this.getTradeButton(name);
			}
			...
			for (let mat in materials) {
				...
				if (tick <= 0) {
					if (name === 'leviathans') {
						...
						if (mat === 'unobtainium' && resMap['burnedParagon'].value < 1e5) {
							continue;
						}
					}
					return false;
		...
		getTradeButton: function (race) {
			let racePanels = this.manager.tab.racePanels;
			for (let i in racePanels) {
				let panel = racePanels[i];
	...
	let storeForSummary = function (name, amount, section) {
		...
		let actSection = activitySummary[section];
		if (actSection === undefined) {
			activitySummary[section] = {};
			actSection = activitySummary[section];
		}

		if (actSection[name] === undefined) {
			actSection[name] = parseFloat(amount);
		} else {
			actSection[name] += parseFloat(amount);
		}
	};
