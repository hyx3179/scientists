...
window.run = function() {
	...
	const i18nData = {
		'zh': {
			...
			'summary.auto.ksHelp': '为了游戏可玩性，并没有给萌新开放过多智能项目，随着猫猫的发展珂学家默认下会越来越智能喵',
		...
		upgrade: function () {
					...
					if (upgrades.techs.limited) {
						...
						switch (name) {
							...
							case 'rocketry':
								...
								break;
							case 'ecology':
								if (titanium < 5000 || start) {continue;}
								break;
							case 'biochemistry':
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					// 缺电过滤碳封存
					if (game.resPool.energyProd - game.resPool.energyCons < 40 || (resMap['titanium'].value < 1000 && resMap['oil'].value > 1e4)) {
	...
	CraftManager.prototype = {
		...
		getPotentialCatnip: function (worstWeather, pastures, aqueducts) {
			...
			let ResCatnip = game.village.getResProduction().catnip;
