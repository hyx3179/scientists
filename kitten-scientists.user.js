		...
		worship: function () {
			...
			if (option.bestUnicornBuilding.enabled) {
				...
				if (btn) {
					...
					// 精炼5悲伤
					if (bls && resMap['tears'].value >= 10e3) {
			...
			// 大教堂
			let basilica = religion.getRU("basilica").on || resMap['gold'] < 750 || resourceFaith.perTickCached < 6
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (!religionRU && game.religion.faith > 2e4 && resMap['gold'].maxValue > 780 && revolutionRatio) {
					temple.max = 25 - Math.min(10, revolutionRatio * 10);
		...
		space: function () {
			...
			// 自动项目
			{
				...
				if (!trigger) {
					...
					if (starchartVal < FiveSattelite && solarRevolution < 2) {builds['sattelite'].max = 0;}
				...
				if (sattelite < 3 + 2 * (solarRevolution > 9.7) && better) {
				...
				if (game["nummon"]['getBestUnobtainiumBuilding']() !== $I("space.planet.piscine.orbitalArray.label") && Array.max < 0) {
					Array.max = 0;
				}
			}
			...
			switch (id) {
				...
				case 'factory':
					...
					if (spaceManufacturing) {
						...
						if (resMap['titanium'].maxValue < 1.3e5 && game.bld.get(id).val && resMap['titanium'].perTickCached < 70) {count = 0;}
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'ship') {
				...
				optionShipVal = (geodesy) ? Math.max(243, optionShipVal) : 70 / Math.max(0.35, Math.log1p(solar));
			...
			if (name === 'plate' && limited && options.auto.craft.items['steel'].enabled) {
				...
				// 有资源回复燃烧时间水晶全部用完铁
				if (options.auto.options.items.hunt.time) {force = true;}
			}
			...
			if (name === 'steel' && limited) {
				...
				if ((resValue > 200 || ironHut) && this.getValueAvailable('plate') / steelValAva < 1.25) {amount = 0;}
