...
window.run = function() {
	...
	Engine.prototype = {
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (upgrades.upgrades.limited) {
					...
					if (priceRatio) {
						noup.push('register');
						// 神经网络过滤
						if (priceRatio < -0.08) {noup.push('neuralNetworks');}
					}
		...
		trade: function () {
			...
			while (trades.length > 0 && maxTrades >= 1) {
				...
				if (name === 'spiders' && Spiders) {minTrades = Math.floor(0.4 * minTrades);}
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			let templePriceRatio = Math.pow(temple.priceRatio + game.getEffect("priceRatio"), temple.val);
			let renaissance = game.prestige.getPerk('renaissance').researched;
			...
			if (name === 'ship' && limited) {
				...
				let oxi = (!workshop.get('oxidation').researched && !Craft.oxidation) || !solar;
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			if (name === 'dragons') {
				prof = (game.space.getProgram('centaurusSystemMission').on && titaniumTri > 0.5) || (resPercent('uranium') < 0.35 && resPercent('iron') > 0.1);
