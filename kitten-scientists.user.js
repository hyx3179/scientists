...
let run = function() {
	...
	Engine.prototype = {
		...
		worship: function () {
			...
			if (Math.min(0.999, Math.max(0.98, PraiseSubTrigger)) <= rate || doAdoreAfterTimeSkip) {
				...
				let paragonFactor = (production < 2) ? 1 + Math.min(2 / game.prestige.getParagonProductionRatio(), 1) : 1;
		...
		upgrade: function () {
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (upgrades.upgrades.limited) {
					...
					if (priceRatio) {noup.push('register');}
		...
		holdFestival: function () {
			...
			if (!(catpowProf && cultureProf && (resMap['parchment'].value >= 5000 || parchProf))) {return;}
		...
		trade: function () {
			...
			for (name in items) {
				...
				if (name === 'dragons' && solarRevolution > 2 && !prof && resPercent('titanium') < 1) {continue;}
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				// falls through
				case 'mansion':
					vitruvianFeline = vitruvianFeline || (!vitruvianFeline && game.workshop.get('geodesy').researched && game.village.sim.kittens.length < 116);
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'ship') {
				...
				optionShipVal = (geodesy) ? Math.max(afterGeodesyNumber) : 45 / Math.max(0.25, Math.log1p(solar));
			...
			if (!game.workshop.get('spaceManufacturing').researched) {
				...
				switch (name) {
					...
					case 'beam':
						...
						break;
					case 'blueprint':
						if (game.getEffect('productionRatio') > 0.2 && !game.science.get('nanotechnology').researched) {stock += 150;}
						break;
				}
			}
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				case 'wood':
					limRat = (0.09 + res.perTickCached < resMap['catnip'].perTickCached / game.workshop.getCraft("wood").prices[0].val && this.getPotentialCatnip()) ? 1 : 0.5 / Math.log10(10 + sloar);
					break;
				case 'beam': {
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			if (name === 'spiders') {
				if (season !== 2) {prof = titaniumTri > 0.4 && resPercent('coal') < 0.5;}
			...
			if (name === 'dragons') {
				prof = game.space.getProgram('centaurusSystemMission').on && titaniumTri > 0.5 || resPercent('uranium') < 0.4;
				doTrade = doTrade && resMap['uranium'].unlocked;
			}
