	...
	Engine.prototype = {
		...
		distribute: function () {
			...
			for (let i = village.jobs.length - 1; i >= 0; i--) {
				...
				if (name === 'scholar') {
					...
					if (!resMap['starchart'].value && !atheism && resMap['science'].value > 5e3 + 1e4 * (resMap['faith'].value > 0)) {maxKS = 0;}
				...
				if (name === 'miner') {
					...
					if (!game.science.get('civil').researched && resMap['kittens'].value < 10) {maxKS = 2;}
		...
		upgrade: function () {
			...
			if (upgrades.missions.enabled && spaceTab.visible) {
				...
				if (subTrigger === 4) {
					if (resMap['starchart'].value > 1e5 && priceRatio < -0.07) {subTrigger = 5;}
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (!science.get('writing').researched && resMap['minerals'].value) {
					...
					workshop.max = 1;
					if (resPercent('minerals') > 0.9) {workshop.max = 2;}
					msgSummary('workshop');
				} else {
				...
				let optimize = ['library','academy','pasture','barn','harbor','oilWell','warehouse','broadcastTower','accelerator','mansion','quarry','aqueduct','chapel', 'lumberMill','biolab'];
		...
		space: function () {
			...
			// 自动项目
			{
				...
				if (!trigger) {
					...
					if (starchartVal < spaceStation || game.challenges.isActive("energy") || !unobtainiumTick) {bldSpaceStation.max = 0;}
		...
		trade: function () {
			...
			let solar = solarRevolution || (challenge && !faithVal) || resMap['gold'].maxValue < 450;
			...
			let transcendence = (Religion.getRU("transcendence").on || !Faith.transcendence.enabled || !game.getEffect('unobtainiumPerTickSpace'));
			...
			switch (id) {
				...
				case 'factory': {
					...
					if (spaceManufacturing) {
						...
						if (!TitaniumCap) {
							if (faVal) {
								if (geodesy && revolution && Workshop.get("nuclearSmelters").researched && faVal < 11 - 5 * vitruvianFeline + 5 * game.getEffect('productionRatio')) {return count;}
								count *= 0.7 + 5 * priceRatio;
								halfCount = true;
								msgSummary('factory');
							} else {
								count = 1;
			...
			['wood','minerals','iron', 'alloy', 'titanium', 'steel', 'gear', 'beam', 'blueprint'].forEach((name) => {
				...
				switch (name) {
					...
					case 'titanium': {
						...
						if (geodesy && resMap['starchart'].value > 300 && (cal || anarchy)) {
							stock += 250;
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'ship': {
					...
					limRat = (game.workshop.get('geodesy').researched && shipValue < 2.5 * shipLimit && solar < 4) ? 0.7 : 0.4;
				...
				case 'alloy': {
					...
					limRat = (steamworks.on < magneto.on || calcinerVal < 2 || resMap['titanium'].value < 20) ? 0.45 : 0.76;
