	...
	Engine.prototype = {
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					if (game.bld.getBuildingExt('factory').meta.val < 3) {
						noop = noop.concat(['factoryLogistics', 'refrigeration', 'barges','carbonSequestration']);
			...
			if (upgrades.buildings.enabled) {
				...
				if (libraryMeta.stage === 0) {
					if (libraryMeta.stages[1].stageUnlocked) {
						...
						if (ratio && items.dataCenter.enabled && resMap['eludium'].value > Math.pow(3, oneP) + 70) {
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (!game.calendar.festivalDays || resMap['spice'].value < 1e3 + 500 * Production || (!sattelite && winterTick < 0)) {
				...
				if (hasLeader && !game.getEffect('unobtainiumPerTickSpace') && resMap['faith'].maxValue < 8000) {
					temple.max = Math.max(21 + 10 * orbitalGeodesy - Production + 5 * priceRatio, 10 * (1 + revolutionRatio));
				}
				// 无、低太阳革命
				...
				if (!theology && science.get('philosophy').researched && goldTri < 0.96 && !atheism) {
					...
					tradepost.max = 21 - 2 * Production - 5 * hasLeader - 0.45 * (resMap['kittens'].value > 43);
					msgSummary('tradepost');
				}
				...
				if (!geodesy && !orbitalGeodesy) {
					...
					if (spaceManufacturing) {
						...
						if (revolutionRatio && game.getEffect('faithRatioReligion') < 0.2) {tradepost.max = 36 + 22 * orbitalGeodesy;}
					}
				}
		...
		space: function () {
			...
			// 自动项目
			{
				...
				if (!trigger) {
					...
					// 流体切割
					if (starchartVal < 3e4 * (1 + solarRevolution * solarRevolution)) {builds['hydrofracturer'].max = 100 * game.getEffect('spaceRatio') - Production - solarRevolution - 23 * blackSky;}
					if (!solarRevolution) {
						builds['hydrofracturer'].max = 0;
						builds['researchVessel'].max = 42 + Production;
						if (!blackSky) {builds['sunlifter'].max = 0;}
					}
					// 香料提取
					if (starchartVal < 1e5) {builds['spiceRefinery'].max = 0;}
					// 太阳提取
					if (resPercent('antimatter') > 0.9) {builds['sunlifter'].max = 0;}
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'ship': {
					...
					limRat = (shipValue < Math.min((1.5 + solar) * shipLimit, 650) && Workshop.get('geodesy').researched && solar < 5) ? 0.7 + 0.15 * (solar < 1) : 0.4;
