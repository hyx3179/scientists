...
let run = function() {
	...
	const i18nData = {
		...
		'zh': {
			...
			'ui.trigger': '触发条件',
			'ui.trigger.set': '输入新的 {0} 触发条件，取值范围为 0 到 1 的小数。（触发条件：当前触发资源 / 触发资源上限）',
		...
		auto: {
			...
			options: {
				...
				items: {
					...
					gameFilter:         {enabled: true,                    misc: true, label: '游戏日志过滤'},
			...
			filter: {
				...
				console: {},
				items: {
					buildFilter:             {enabled: false,  label: i18n('filter.build'),      variant: "ks-activity type_ks-build"},
					researchFilter:          {enabled: false,  label: i18n('filter.research'),   variant: "ks-activity type_ks-research"},
					upgradeFilter:           {enabled: false,  label: i18n('filter.upgrade'),    variant: "ks-activity type_ks-upgrade"},
					craftFilter:             {enabled: false,  label: i18n('filter.craft'),      variant: "ks-activity type_ks-craft"},
					policyFilter:            {enabled: false,  label: i18n('filter.policy'),     variant: "ks-activity type_ks-policy"},
					upgBldFilter:            {enabled: false,  label: i18n('filter.upgBld'),     variant: "ks-activity type_ks-upgBld"},
					tradeFilter:             {enabled: false,  label: i18n('filter.trade'),      variant: "ks-activity type_ks-trade"},
					embassyFilter:           {enabled: false,  label: i18n('filter.embassy'),    variant: "ks-activity type_ks-embassy"},
					huntFilter:              {enabled: false,  label: i18n('filter.hunt'),       variant: "ks-activity type_ks-hunt"},
					// 赞美太阳 faith => parise
					faithFilter:             {enabled: false,  label: i18n('filter.praise'),     variant: "ks-activity type_ks-praise"},
					adoreFilter:             {enabled: false,  label: i18n('filter.adore'),      variant: "ks-activity type_ks-adore"},
					transcendFilter:         {enabled: false,  label: i18n('filter.transcend'),  variant: "ks-activity type_ks-transcend"},
					unicornSacrificeFilter:  {enabled: false,  label: i18n('filter.sacrifice'),  variant: "ks-activity type_ks-sacrifice"},
					faithBuildFilter:        {enabled: false,  label: i18n('filter.faith'),      variant: "ks-activity type_ks-faith"},
					accelerateFilter:        {enabled: false,  label: i18n('filter.accelerate'), variant: "ks-activity type_ks-accelerate"},
					tcShatterFilter:         {enabled: false,  label: i18n('filter.time.skip'),  variant: "ks-activity type_ks-timeSkip"},
					festivalFilter:          {enabled: false,  label: i18n('filter.festival'),   variant: "ks-activity type_ks-festival"},
					astronomicalEventFilter: {enabled: false,  label: i18n('filter.star'),       variant: "ks-activity type_ks-star"},
					distributeFilter:        {enabled: false,  label: i18n('filter.distribute'), variant: "ks-activity type_ks-distribute"},
					leaderFilter:            {enabled: false,  label: i18n('filter.leader'),     variant: "ks-activity type_ks-leader"},
					miscFilter:              {enabled: false,  label: i18n('filter.misc'),       variant: "ks-activity"}
				}
			},
			resources: {
	...
	Engine.prototype = {
		...
		delay: function (render) {
			if (render) {
				...
				if (!tooltip) {return this.tooltip = dojo.byId('tooltip');}
		...
		timeCtrl: function () {
			...
			if (optionVals.timeSkip.enabled && game.workshop.get('chronoforge').researched) {
				...
				if (heatNow - 15 * game.getEffect('heatPerTick') < 0) {
		...
		distribute: function () {
			...
			if (leaderVals.enabled && game.science.get('civil').researched && !game.challenges.isActive("anarchy") && !options.copyTrait) {
				...
				if (traitName === 'none' && leaderJobName === 'engineer') {leaderVals.leaderTrait = 'manager';}
			...
			for (let i = village.jobs.length - 1; i >= 0; i--) {
				...
				if (name === 'hunter') {
					...
					if (manpowerJobRatio < 0.5) {
						...
						if (!expect && !activitySummary.other['auto.hunter']) {
				...
				if (name === 'scholar' && limited) {
					...
					if (resPercent('science') > 0.6) {continue;}
				}
		...
		_worship: function (builds) {
			...
			if (!gReligion.getRU("basilica").on && copyBuilds['basilica'].enabled) {
				if (!game.religion.getRU("sunAltar").on) {copyBuilds['goldenSpire'].enabled = false;}
			...
			if (!game.ironWill && !game.workshop.get('orbitalGeodesy').researched) {copyBuilds['templars'].enabled = false;}
			if (gReligion.getSolarRevolutionRatio() > 9.98 + 0.9 * game.getEffect("solarRevolutionLimit") && game.workshop.get('spaceManufacturing').researched && activitySummary.other.adore) {
		...
		upgrade: function () {
			...
			if (upgrades.techs.enabled && game["libraryTab"].visible) {
				...
				for (let upg of tech) {
					...
					if (upgrades.techs.limited) {
						...
						if (name === 'biology' && resMap['compedium'].value < 2e3) {continue;}
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (upgrades.upgrades.limited) {
					...
					if (game.bld.get('steamworks').on < 6 && !game.challenges.isActive("pacifism")) {
					...
					if (magneto.on === magneto.val) {noup.push('carbonSequestration');}
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				// 粮仓
				if (spaceManufacturing) {items['barn'].max = -1;}
				//图书馆牧场
				...
				if (zigguratM.val > 10 && resMap['blueprint'].value < 2 * factor && !spaceManufacturing) {items['ziggurat'].max = 0;}
				// 天文台
				...
				if (!science.get('metalurgy').researched) {Brewery.max = 5;}
				...
				if (!religionRU && game.religion.faith > 1e4 && resMap['gold'].maxValue > 780 && revolutionRatio) {
					temple.max = 22 - Math.min(10, revolutionRatio * 10);
				...
				if (orbitalGeodesy) {
					if (!spaceManufacturing || !sattelite) {
						calciner.max = (calcinerMax === -1) ? Math.min(47 + 47 * game.getEffect("productionRatio"), 65 + 10 * blackSky)
		...
		space: function () {
			...
			if (!trigger) {
				...
				if (starchartVal < fourSattelite) {builds['spaceStation'].max = 0;}
				if (resPercent('unobtainium') < 0.8) {builds['moonBase'].max = 0;}
				else {builds['moonBase'].max = game.space.getBuilding('moonBase').val + 1;}
				if (!game.getEffect('lunarOutpostRatio')) {builds['spaceElevator'].max = 11;}
			}
			if (game.ironWill) {builds['spaceStation'].max = 0;}
			if (game.space.getBuilding('sattelite').val < 3 + 2 * (solarRevolution > 9.7) && better) {
		...
		trade: function () {
			...
			let skipNagas = game.calendar.year > 2e3 || (renaissance && resMap['alicorn'].value);
			...
			// 节日
			if (optionTrade.festival) {return;}
			// if (craftManager.getTickVal(resMap('parchment'), true) > 15)
		...
		miscOptions: function () {
			...
			if (optionVals.gameFilter.enabled && filter.enabled) {
		...
		getBestUnicornBuilding: function () {
			...
			let ivory = resMap['tears'].value + unicornsMap.value * 2500 / onZig > fa * resMap['ivory'].value;
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'oilWell':
					if (id === 'oilWell') {
						if (game.bld.getBuildingExt(id).meta.val < 8 || resMap['oil'].maxValue < 2e4) {break;}
				...
				case 'factory':
					...
					if (spaceManufacturing) {
						if (!game.space.getBuilding('sattelite').val && game.bld.get(id).val) {
							options.auto.space.times['sattelite'].enabled = true;
							count = 0;
						}
					} else {
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'ship') {
				...
				optionShipVal = (geodesy) ? Math.max(243, optionShipVal) : 100 / Math.max(0.5, Math.log1p(solar));
			...
			if (name === 'beam' || name === 'slab') {
				...
				if (resPercent(i) < 2) {
					options.auto.craft.autoConsume[i] = 0;
					if (resMap['scaffold'].value > 500 && trigger > 0.9 && limited) {
						options.auto.craft.autoConsume[i] = 1;
					}
					if (!resMap[name].value && resMap['slab'].value) {force = true;}
				}
			}
			if (name === 'megalith'&& limited && resMap[name].unlocked && game.diplomacy.get('nagas').embassyLevel > 15 && !game.calendar.season) {
				autoMax = 1;
				force = true;
			}
			...
			if (name === 'plate' && limited && options.auto.craft.items['steel'].enabled) {
				...
				if (coalTick > 0 && resMap['iron'].value !== resMap['iron'].maxValue && resValue > 150) {
			...
			if (name === 'parchment') {
				...
				if (!gScience.get('navigation').researched) {
			...
			if (name === 'alloy' && limited) {
				let forceAlloy = (name, price) => {
					...
					let oxidation = options.auto.craft.oxidation && !game.workshop.get('oxidation').researched;
					if (workshopMeta.researched || !workshopMeta.unlocked || oxidation) {return;}
		...
		getStock: function (name) {
			...
			if (name === 'steel') {
				...
				let steelSaw = unResearched('steelSaw') && resMap[name].value > 350;
				if (steelSaw) {
					stock += 750;
				}
			...
			if (name === 'gear') {
				if (unResearched('rotaryKiln') && workshop.get('orbitalGeodesy').researched) {stock += 5000;}
				if (unResearched('fuelInjectors') && resPercent('coal') < 0.8 && resMap['oil'].value > 2e4) {stock += 250;}
			}
			if (name === 'beam') {
		...
		getLimRat: function (name, limited, limRat) {
			...
			if (name === 'slab') {
				...
				limRat = (resMap['scaffold'].value > 2500) ? 0.02 : limRat;
			}
			...
			if (name === 'ship') {
				...
				limRat = (shipValue > shipLimit * 0.75 && sloar > 3) ? 0.3 : limRat;
				limRat = (shipValue > shipLimit * 5 || manufacture) ? 0.05 : limRat;
			...
			if (name === 'plate') {
				...
				limRat = ((res.value < 150 && navigation.researched && resMap['starchart'].value > 15 && resMap['scaffold'].value > 50)
					|| titan) ? 0.9 : limRat;
			...
			if (name === 'alloy') {
				...
				limRat = (steamworks.on < magneto.on || titaniumTick === 5e-4) ? limRat : 0.75;
				let factor = Math.pow(magneto.priceRatio + priceRatio, magneto.val);
				limRat = (res.value > Math.max(1250, 10 * factor) && options.auto.build.items.magneto.enabled) ? 0.01 : limRat;
				limRat = (resMap['titanium'].value < 19 && !titaniumTick) ? 0 : limRat;
			}
			...
			if (name === 'concrate') {
				...
				let factor = Math.pow(reactor.priceRatio + priceRatio, reactor.val);
				limRat = (reactor.val) ? limRat : 0.3;
				limRat = (res.value > Math.max(moon, 50 * factor)) ? 0.01 : limRat;
				limRat = (!res.unlocked && game.diplomacy.get('nagas').embassyLevel > 10 && !game.calendar.season && resMap['titanium'].value > 500) ? 1 : limRat;
			}

			if (name === 'gear') {
				let factor = Math.pow(steamworks.priceRatio + priceRatio, steamworks.val);
			...
			if (name === 'manuscript') {
				...
				limRat = (resMap['parchment'].value > Math.max(1e6, 250 * priceRatio) && reactor.val) ? 1 : limRat;
			}
			...
			if (name === 'scaffold') {
				...
				let factor = Math.pow(observatory.priceRatio + priceRatio, observatory.val);
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			let materials = this.getMaterials(name);
			let cost = 0;
			if (name === 'leviathans' && game.time.getCFU("ressourceRetrieval").val && resPercent('unobtainium') > 0.7) {return true;}
			for (let mat in materials) {
			...
			if (name === 'nagas') {
				...
				if (resMap['concrate'].value < 500 && race.embassyLevel > 10 && resMap['titanium'].value > 1000) {tick = true;}
			...
			if (name === 'zebras') {
				...
				if (griffins.enabled && griffins.autumn && game.calendar.season === 2) {
					prof = prof && autumnIron && resPercent('titanium') < 0.95 && titaniumProfit > titanium.perTickCached;
				}
				let solar = game.religion.getSolarRevolutionRatio();
			...
			return doTrade || tick === true || (cost <= profit && prof);
		},
	...
	if (ksUi) {
		...
		let optionsTitleElement = $('<a/>', {
			css: { display: 'inline-block', textShadow: '3px 3px 4px gray', fontStyle:'italic', transform: 'scale(0.85)'},
			text: version,
			target: '_blank',
	...
	let displayActivitySummary = function () {
		...
		if (!filter.enabled || !filter.items['craftFilter'].enabled || !filter.items['buildFilter'].enabled
			...
			msg = game.msg('喵喵提示：游戏(珂学家)日志消耗性能比较多');
			$(msg.span).css('color', "#ff589c");
		}
	...
	let engineOn = () => {
		...
		toggleEngine.on('change', function () {
			...
			if (toggleEngine.is(':checked')) {
				...
				msgStock();
				setTimeout(()=>{
					if (Engine.enabled) {game.msg('点击小猫总结可以查询干了什么', 'notice');}
				}, 2000);
			} else {
