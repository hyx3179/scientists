...
window.run = function() {
	...
	const i18nData = {
		'zh': {
			...
			'summary.auto.biology': '喵喵喵嫌弃钛少了，就不研究生物学',
			...
			'summary.auto.drama': '喵力产量低，羊皮纸库存低，聪明的小猫就不过节日啦',
			...
			'summary.auto.ksHelp3': '默认设置下纯挂机大概450年左右130猫口+新约外传',
			...
			'summary.auto.moonBase': '难得素储量达到80%，小猫才会造月球基地',
		...
		auto: {
			...
			options: {
				...
				items: {
					...
					hunt:               {enabled: true,  subTrigger: 0.96, require: 'manpower', misc: true, label: i18n('option.hunt')},
	...
	Engine.prototype = {
		...
		timeCtrl: function () {
			...
			if (optItem.accelerateTime.enabled && game.science.get("calendar").researched) {
				...
				if (tf.value >= Math.max(tf.maxValue * optItem.accelerateTime.subTrigger, 1) && !game.time.isAccelerated) {
		...
		distribute: function () {
			...
			let negative = 4000 * (catnipTick - 0.85 * (happiness * !anarchy - 1));
			...
			if (leaderMeta.enabled && game.science.get('civil').researched && !anarchy && !options.copyTrait) {
				...
				if (force && traitName === 'manager' && tt > 6 && village.getJob('priest').unlocked) {
				...
				if (Leader == null || !(Leader.job === leaderJobName && Leader.trait.name === traitName)) {
					...
					if (traitKittens.length !== 0) {
						if (distributeJob.unlocked && distributeJob.value < village.getJobLimit(leaderJobName)) {
							...
							if (correctLeaderKitten.job === 'farmer') {
								game.village.sim.removeJob('scholar', 1);
			...
			for (let i = village.jobs.length - 1; i >= 0; i--) {
				...
				if (name === 'geologist') {
					...
					if (tt > 7 && !anarchy) {maxKS = Math.min(maxKS, 30);}
				...
				if (name === 'scholar' && limited) {
					let moreScholar = 0.29;
					if (!game.getEffect('shatterTCGain') && scholar && revolution) {
						moreScholar = 0.6;
					}

					if (resPercent('science') > moreScholar && val || !woodcutter) {
						maxKS = 0;
					} else if (moreScholar > 0.29) {
				...
				if (name === 'priest') {
					...
					if (!revolution && tt > 3 && maxKS === 3 && val < 3) {val = 0;}
					if (!woodcutter) {
						limited = true;
						maxKS = 0;
					}
		...
		worship: function () {
			...
			if (Math.min(0.999, Math.max(0.98, PraiseSubTrigger)) <= rate || doAdoreAfterTimeSkip) {
				...
				if (option.transcend.enabled && transcendenceReached) {
					...
					if (tt < 11 && moonBoolean && worship > 1e5 && apocripha && transcendCatnip) {
						TranscendTimes = 4;
					}
			...
			// 彩色玻璃
			let glass = stainedGlassOn || resMap['gold'].value < 200 || game.getEffect('culturePerTickBase') < 0.9;
			// 大教堂
			let basilica = religion.getRU("basilica").on || resMap['gold'].value < 675 || resourceFaith.perTickCached < 2.8
		...
		_worship: function (builds) {
			...
			let Unlocked = (Religion.faith > solarMeta.faith || game.prestige.getPerk("voidOrder").researched) && resMap['faith'].maxValue > 750 && resMap['gold'].maxValue > 470;
			let tt = Religion.transcendenceTier;
			let trigger = (!solarMeta.on && Unlocked && Religion.getRU('solarchant').on && tt) ? 1.1 : options.auto.faith.trigger;
			...
			if (!Religion.getRU("sunAltar").on && basilica) {copyBuilds['goldenSpire'].enabled = false;}
			if (!basilica && copyBuilds['basilica'].enabled) {
				let glass = game.religion.getRU("stainedGlass").on;
				if (game.religion.faith > 5e4) {
					if (game.village.happiness > 2.4) {copyBuilds['sunAltar'].enabled = false;}
			...
			if (!game.ironWill && game.getEffect('faithRatioReligion') < 0.2) {
				...
				if (!Religion.getRU("templars").on && resMap['faith'].value > 3e3) {msgSummary('templars');}
			...
			// 新约外传
			if (tt && !Religion.getRU("transcendence").on) {copyBuilds['apocripha'].enabled = false;}
		...
		upgrade: function () {
			...
			if (upgrades.techs.enabled && game["libraryTab"].visible) {
				...
				let acoustics = !game.science.get('acoustics').researched && resMap['science'].value > 6e4 && game.religion.transcendenceTier && !titanium;
				let relic = resMap['relic'].value;
				let start = resMap['starchart'].value > 1e5 && !game.science.get('drama').researched;
				techLoop:
				for (let upg of tech) {
					...
					if (upgrades.techs.limited) {
						...
						switch (name) {
							...
							case 'ai':
								...
								break;
							case 'physics':
								if (acoustics && (resMap['compedium'].value < 120 || resMap['steel'].value < 100)) {continue;}
								break;
							...
							case 'oilProcessing':
								...
								break;
							case 'drama':
								if (craftManager.getTickVal(resMap['parchment'], true) < 3 && resMap['parchment'].value < 13e4) {
									msgSummary('drama');
									continue;
								}
								break;
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					let isFilter;
					noop = ['biofuel','gmo','invisibleBlackHand'];
					if (game.getEffect('shatterTCGain') < 0.05) {noop.push('steelPlants');}
					...
					// 太阳革命
					if (!revolutionRatio && resMap['faith'].maxValue >= 750 && game.religion.faith > 1000) {noop.push('caravanserai');}
					// 缺电过滤碳封存
					if (game.resPool.energyProd - game.resPool.energyCons < 40) {
					...
					if (isFilter && !game.space.meta[0].meta[3].val && resStarchart.value < 1600) {
						noop = noop.concat(['geodesy','titaniumWarehouses','titaniumBarns']);
						if (!orbitalGeodesy) {
							if (game.getEffect('calcinerRatio') > 1) {noop.push('fuelInjectors');}
							if (resMap['oil'].value > 35e3) {noop.push('pumpjack');}
						}
					}
					if (!geodesy) {
						// 冷藏
					...
					if (game.bld.getBuildingExt('aiCore').meta.val < 5) {
						...
						if (Production > 4) {
							noop = noop.concat(['stoneBarns', 'reinforcedBarns', 'reinforcedWarehouses', 'titaniumBarns', 'alloyBarns', 'concreteBarns', 'titaniumWarehouses', 'alloyWarehouses', 'concreteWarehouses', 'storageBunkers', 'energyRifts', 'stasisChambers', 'voidEnergy', 'darkEnergy']);
						}
					}
					// 反应堆槽
					...
					// 钍反应堆
					if (resMap['thorium'].value < 1.1e5 || game.resPool.energyProd - game.resPool.energyCons > 400) {noop.push('thoriumReactors');}
					// 天体观测仪
					isFilter = resMap['science'].maxValue > 16e4 && resStarchart.value < 2075;
			...
			if (upgrades.missions.enabled && spaceTab.visible) {
				...
				if (subTrigger === 4 && (resMap['starchart'].value > 1e5 * (1 + Production) * (1 + revolutionRatio) || resMap['alicorn'].value)) {
					subTrigger = 7;
				...
				if (aqueductMeta.stage === 0) {
					if (aqueductMeta.stages[1].stageUnlocked) {
						if (pastureMeta.stage === 1) {
							let titanium = 250 * Math.pow(pastureMeta.stages[1].priceRatio + priceRatio, pastures.val) > resMap['titanium'].maxValue
							|| energyActive;
							let catnipTick = craftManager.getPotentialCatnip(true, pastures, 0);
							let catnip = catnipTick > 0 || (catnipTick < 0 && resMap['catnip'].value + 5e4 * catnipTick > 0);
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				if (!game["workshopTab"].visible && science.get('animal').researched) {
					smelter.max = 0;
					if (pasture.max === -1 && !game.challenges.isActive('winterIsComing')) {
				...
				if (theology) {
					...
					if (Production > 2.8 && winterTick > 4.5 && hutVal < 4 && Religion.transcendenceTier > 10 && hutVal > 1 && !science.get('construction').researched) {
						pasture.max = 0;
						items['barn'].max = 0;
						if (hutVal === 2) {
							items['library'].max = 18;
							items['mine'].max = 0;
						}
						if (hutVal === 3) {
							items['library'].max = 30;
						}
					}
					if (machinery && resPercent('wood') < 0.9 && hasLeader) {items['library'].max = 45;}
				}
				...
				if (zigguratM.val > 10 && resMap['blueprint'].value < 2.5 * factor && !resMap['alicorn'].unlocked) {items['ziggurat'].max = 0;}
				// 天文台
				if (blackSky && resMap['science'].maxValue > 3e5 && !orbitalGeodesy) {items['observatory'].max = 3;}
				...
				if (!game.calendar.festivalDays || resMap['spice'].value < 1e3 + 500 * Production) {Brewery.max = 0;}
				...
				if (!revolutionRatio) {
					...
					if (smelterVal) {
						if (resMap['wood'].perTickCached * 2 < smelterVal || resMap['minerals'].perTickCached < smelterVal) {
							smelter.max = 0;
							msgSummary('smelter');
						}
					}
				...
				let expect = options.auto.faith.addition.autoPraise.expect;
				if (geodesy) {
					if (hasLeader && !orbitalGeodesy && game.bld.get('hut').val < 22 && game.workshop.get('concreteHuts').researched) {
						buildManager.built('hut', undefined, 1);
						buildManager.built('logHouse', undefined, 1);
					}
					msgSummary('smelter', true);
				} else {
					let religionRU = !Religion.getRU("sunAltar").on || !Religion.getRU("sunAltar").on || !Religion.getRU("sunAltar").on
						|| (revolutionRatio > 3 && !sattelite && game.getEffect('faithRatioReligion') < 0.2);
					if (religionRU && revolutionRatio) {
						temple.max = 24 - 3 * Production - 2 * (revolutionRatio > 3) - 3 * hasLeader;
						tradepost.max = 19 + 2 * templeVal - 5 * Production - 3 * Math.sqrt(revolutionRatio) - 2 * hasLeader;
						msgSummary('religion');
					}
					// 测地学 交易所
					if (!revolutionRatio) {
						tradepost.max = 30 - 20 * (Production > 4) - 5 * revolutionRatio;
					}
				}
				// 太阳革命前不造交易所和神殿
				if (!solarMeta.on && !atheism && theology) {
					if (options.auto.faith.items.solarRevolution.enabled && resMap['faith'].maxValue > 750 && resMap['gold'].maxValue > 530) {
						if (science.get('philosophy').researched) {
							msgSummary('temple');
							temple.max = Math.floor(7.5 / (1 + game.prestige.getParagonStorageRatio())) + 2 * !hasLeader;
							if (resMap['gold'].value > 530) {
								temple.max = templeVal + 1;
							}
						}
					}
					if (Religion.transcendenceTier) {
						tradepost.max = 18 - 2 * Production + 1 * resMap['kittens'].value < 60 - 2 * hasLeader;
				...
				if (!theology && science.get('philosophy').researched && goldTri < 0.96 && !atheism) {
					...
					tradepost.max = 20 - 2 * Production - 4 * hasLeader;
					msgSummary('tradepost');
				}
				...
				if (orbitalGeodesy) {
					if (!spaceManufacturing || !sattelite) {
						let Max = Math.min(47, 6 * revolutionRatio + 15 + 2 * Production);
					...
					if (!game.workshop.get('oxidation').researched) {calciner.max = 5;}
				}
				...
				if (game.space.getProgram('moonMission').on && resPercent('titanium') > 0.8) {
					if (resPercent('titanium') !== 1) {
				...
				// 挑战等3个传送仪一起造
				if (priceRatio && resMap['unobtainium'].value < 8794 && game.calendar.futureSeasonTemporalParadox < 0 && !game.getEffect('resStasisRatio')) {
		...
		space: function () {
			...
			// 自动项目
			{
				...
				let blackOrSolar = blackSky || solarRevolution > 5 || game.workshop.get('spaceManufacturing').researched;
				...
				if (!trigger) {
					...
					if (starchartVal < spaceStation || game.challenges.isActive("energy")) {builds['spaceStation'].max = 0;}

					// 电梯
					if (!game.getEffect('lunarOutpostRatio') && unobtainiumTri !== 1) {
						builds['spaceElevator'].max = 8 + Production;
						if (!vitruvianFeline && unobtainiumTri < 0.5) {builds['spaceElevator'].max = 0;}
					...
					// 月球前哨
					if (game.getEffect('unobtainiumPerTickSpace') && uranium < 0.7) {builds['moonOutpost'].max = 0;}
					// 月球基地
					...
					// 星球裂解
					if (uranium > 10 && starchartVal < 1e6) {builds['planetCracker'].max = 0;}
					// 流体切割
		...
		holdFestival: function () {
			...
			let noSkip = resMap['parchment'].value >= 7500 || parchProf || resMap['manpower'].perTickCached > 18;
		...
		hunt: function () {
			...
			manpower.value += manpower.perTickCached * 2 * (subTrigger > 0.9);
			let huntCount = Math.floor(manpower.value / 100);
			let architecture = game.science.get('writing').researched;
			let preCount = (!resMap['furs'].value || resMap['parchment'].value < 25 || game.ironWill) ? Math.max(30, Math.floor(0.5 * huntCount)) : 0;
		...
		trade: function () {
			let cacheSummary = {};
			let ks = !activitySummary.other['auto.ksHelp'];
			let Ratio = (activitySummary.other['auto.changeLeader']) ? 0 : 1 + ks;
			...
			let apocripha = (game.religion.getRU('apocripha').on || !Faith.apocripha.enabled || game.religion.faithRatio > 0.22 || game.religion.faith < 1e5);
			...
			for (name in items)  {
				...
				if (name === 'dragons' && solarRevolution > 2 && titaniumTri < 0.9 && resPercent('uranium') > 0.1) {continue;}
				if (name === 'lizards' && solarRevolution > 0.2) {continue;}
				if (name === 'sharks' && solarRevolution > 1 && !challenge) {continue;}
				trade = items[name];
				...
				if ((!require || requireTrigger <= require.value / require.maxValue) && isGoldTrigger) {
					...
					if (tradeManager.getProfitability(name)) {
						trades.push(name);
						if (name !== 'leviathans') {isLimited = true;}
					}
				}
			}

			let catnipNow = resMap['catnip'].value + 100 * game.getResourcePerTick("catnip", true);
			if (catnipNow < 0 && game.deadKittens < 1e3 && !game.ironWill) {
			...
			let oneTrade = Calendar.season === 2 && titaniumTri > 0.5;
			const tradesDone = {};
		...
		miscOptions: function () {
			...
			if (optItem.buildEmbassies.enabled && !!game.diplomacy.races[0].embassyPrices) {
				...
				let b = cultureTri > Math.max(0.96, options.auto.craft.trigger);
				let solarFactor = Math.sqrt(game.religion.getSolarRevolutionRatio() + 1);
				let cultureForce = culture.value > 5e3 + 5e3 * (tt > 5) + 5e3 * Math.sqrt(2 * game.religion.getSolarRevolutionRatio());
				if (optItem.buildEmbassies.subTrigger <= cultureTri && cultureTri < 2 || cultureForce) {
					...
					while (bulkTracker.length > 0) {
						for (i = 0; i < bulkTracker.length; i++) {
							...
							let noSkip = true;
							let Sharks = name === 'sharks' && solarFactor > 2 && !game.getEffect('unobtainiumPerTickSpace') && resMap['furs'].value;
							let Dragons = name === 'dragons' && !game.getEffect('unobtainiumPerTickSpace');
							let priority = embassyVal > 14 + solarFactor - 10 * (name === 'zebras') && astronomy && !b;
							if (priority || Dragons || Sharks) {noSkip = false;}
			...
			const list = [{
				...
				conditionOff: game.bld.get('brewery').on && (!game.calendar.festivalDays || !spiceVal),
			}];
			// 自动控制 时间操纵 酿酒厂 开关
			if (!game.opts.enableRedshift) {
				// 开启离线进度时不调整时间操纵
	...
	UpgradeManager.prototype = {
		...
		getValue: function (name, upgrade) {
			...
			if (name === 'titanium' && upgrade !== 'rotaryKiln') {
				...
				if (cache === 'orbitalGeodesy' && cache !== upgrade && upgrade !== 'pumpjack') {stock += 1e3;}
			}
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'chapel':
					if (id === 'chapel') {
						if (game.bld.getBuildingExt(id).meta.val < 17 + revolution) {
							break;
						} else {
				...
				case 'aqueduct': {
					...
					if (catnipTick < 2 || game.challenges.isActive('winterIsComing')) {
						break;
				...
				// falls through
				case 'mine':
					if (id === 'mine' && vitruvianFeline) {
						if (game.bld.getBuildingExt('logHouse').meta.val || game.ironWill) {
							break;
						}
					}
				// falls through
				case 'oilWell':
					if (id === 'oilWell') {
						...
						if (meta < 9 || oil.maxValue < 2e4) {break;}
						if (oil.maxValue > 55e3 && Val > 3e4 && meta < 199) {count = Math.floor(count * 0.5);}
						if (game.getEffect("calcinerRatio") > 1 && !orbitalGeodesy && Val < 3e4) {return count;}
					}
				// falls through
				case 'quarry' :
					if (vitruvianFeline) {
				...
				case 'harbor':
					if (id === 'harbor') {
						...
						if (harborVal < 8) {
							msgSummary('harbor');
							halfCount = true;
						} else if (reactorVal < 20 && revolution > 7.5) {
							halfCount = true;
				...
				case 'logHouse':{
					...
					if (am.val < 5 && !am.stage && am.unlocked && game.village.happiness < 3 && game.village.maxKittens > 18) {return 0;}
					break;
				}
				case 'biolab':
					if (!spaceManufacturing) {
			...
			if ((id === 'hut' || id === 'logHouse') && count) {
				...
				let noFarmer = Catnip.value < 5e3 + happy * 100 && Catnip.perTickCached < 1.7 * happy && (!game.science.get('agriculture').researched || anarchy);
				let noKitten = noFarmer || (catnipTick < 1 && Catnip.value + catnipTick * 1000 - 1000 < 0);
				if (noKitten && !geodesy) {
					count = 0;
					msgSummary('kittens');
				} else {
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
				...
				if (force && oxi) {
					if (geodesy) {
						...
						if (game.religion.faithRatio) {
							if (tt < 6 && geodesy) {forceShipVal = -100 * tt + 750 - 250 * !tt;}
						}
					}
				}
				force = force && shipValue < forceShipVal && oxi && ratio > 3.3;
			}
			...
			if (name === 'megalith'&& limited) {
				let five = resMap[name].unlocked && game.diplomacy.get('nagas').embassyLevel > 15
				&& !game.calendar.season && !resValue;
				let HighCraft = ratio > 10 && resValue < 40;
				if (HighCraft || five) {
					autoMax = 1;
					force = true;
				}
			...
			if (name === 'plate' && limited) {
				...
				if (!shipValue && ratio > 3 && resMap['starchart'].value > 24 && resValue < 150) {
			...
			if (name === 'manuscript' && limited) {
				...
				if (cultureTri > 0.9 && cultureTri < 2 && (resMap['manpower'].perTickCached < 14 || !navigation) && resValue < 1e3) {
					force = true;
				}
				if (!game.calendar.festivalDays && resMap['unobtainium'].value && Science.get('drama').unlocked) {autoMax = 0;}
			}

			let religion = resMap['gold'].maxValue < 500 || game.religion.getRU('solarRevolution').on || game.religion.transcendenceTier;
			if (name === 'compedium' && limited && navigation && religion) {
				...
				// 物理学 => 强制到声学
				if (i === 20 && resMap[name].value < 60 && game.religion.transcendenceTier) {
					force = true;
					autoMax = 1;
				}
				// 冶金
				if (!Science.get('metalurgy').researched && cultureTri > 0.98 && resValue < 100 && scienceTri > 0.98) {
					force = true;
				}
			...
			if (name === 'parchment') {
				limited = ratio < 2.1;
				resValue = resMap[name].value;
				if (ratio > 3 && resMap['furs'].value < 100) {return;}
				if (resValue < 5 && ratio > 1.5) {
					force = true;
					autoMax = 1;
				}
				if (!navigation) {
			...
			if (name === 'steel' && limited) {
				let calciner = game.bld.getBuildingExt('calciner').meta;
				let steelVal = resMap['steel'].value;
				...
				if (resMap['plate'].value < 1.25 * ironCoalRatio * resMap['steel'].value && !Craft.oxidation && Craft.items['plate'].enabled && steelValAva) {amount = 0;}
				...
				if (calVal) {
					...
					if (oxid) {
						...
						let T = 6000 - 1500 * !geodesy + 1000 * (calVal > 4);
					...
					if (flu && !cacheUpg.cache) {
						...
						// 2分钟 5 * 60 * 2
						let b = titanium > amt * 10 || resMap['alloy'].value > 100 || (amt * 10 - titanium) / resMap['titanium'].perTickCached < 600 || calVal > 14;
						if (amt > 0 && calVal > 6 && b) {
			...
			if (name === 'alloy' && limited) {
				let forceAlloy = (name, price) => {
					...
					if (amt > 1 && resMap['steel'].value > amt * 75 && resMap['titanium'].value > amt * 10 && !Craft.oxidation) {
						amount = amt;
						force = true;
						if (game.religion.getSolarRevolutionRatio() < 7 || price > 25) {activity(i18n('craft.forceSteel', [workshopMeta.label]));}
					}
				};
		...
		getTickVal: function (res, preTrade) {
			...
			if (!preTrade) {
				let ratio = 1;
				if (name === 'unobtainium' && activitySummary['other']['time.skip']) {ratio = 0.4;}
		...
		getStock: function (name) {
			...
			if (!stocks) {
				this.getAutoStocks();
				stocks = options.auto.cache.stocks;
			}
			if (stocks[name]) {
				stock += stocks[name];
			}
		...
		getAutoStocks: function () {
			...
			let cacheAlloy = options.auto.cache.resUpg['alloy'];
			...
			['wood','iron', 'alloy', 'steel', 'titanium', 'gear', 'beam', 'blueprint'].forEach((name) => {
				...
				switch (name) {
					case 'wood': {
						...
						if (game.bld.get('hut').val < 23 && workshop.get('concreteHuts').researched && !iw && !game.getEffect("priceRatio") && !resMap['uranium'].value && resPercent(name) < 1) {stock += 587e3;}
						break;
					}
					case 'iron': {
						...
						if (game.bld.getBuildingExt('temple').meta.on > 2 || resMap['faith'].maxValue > 749 || a || resMap['plate'].value > 18) {
							let stockIron = (isStock, price, upg) => {
								if (isStock && resMap['iron'].value < 1e4 && kittens < 115) {
							...
							let ironwood = this.getUnResearched('ironwood') && resMap[name].value > 750 && resMap[name].maxValue > 3000
					...
					case 'steel': {
						...
						if (steelAxe && !iw && (game.bld.get('lumberMill').val > 30 || resMap[name].value > 10)) {msgForStock(75, 'steelAxe', name);}
						// 精钢锯
					...
					case 'titanium': {
						...
						// 回转炉
						if (this.getUnResearched('rotaryKiln') && ((Val > 20 && workshop.get('geodesy').researched) || orbGeodesy)) {msgForStock(5000, 'rotaryKiln', name);}

						let Revolution = game.religion.getSolarRevolutionRatio();
						let cal = Revolution < 2.5 && (Val > 2 || resMap['titanium'].value > 100) && leader;
						let anarchy = game.challenges.isActive("anarchy") && Revolution < 6;
						let geodesy = this.getUnResearched('geodesy');
						if (geodesy && resMap['starchart'].value > 450 && (cal || anarchy)) {stock += 250;}
						// 混凝土小屋
						if (this.getUnResearched('concreteHuts') && !geodesy && resMap['titanium'].value > 500 && !iw && resMap['concrate'].value > 45 && resMap['science'].maxValue > 12e4 && resMap['uranium'].value < 250) {stock += 3e3;}
					...
					case 'gear':
						if (this.getUnResearched('rotaryKiln') && resMap['titanium'].value > 4e3) {stock += 500;}
						if (this.getUnResearched('fuelInjectors') && resMap['oil'].value > 2e4 && game.getEffect('calcinerRatio') < 1) {stock += 250;}
						// 抽油机
						if (this.getUnResearched('pumpjack') && resMap['titanium'].value > 200 && resPercent('oil') < 0.8) {stock += 125;}
						if (this.getUnResearched('logistics') && resMap['scaffold'].value > 1000 && leader) {stock += 100;}
						// 石油加工
						if (this.getUnResearched('oilRefinery') && resMap['titanium'].value > 1250) {stock += 500;}
						break;
					case 'beam':
						if (this.getUnResearched('deepMining') && resMap['iron'].value > 1200) {stock += 5000;}
						break;
					case 'blueprint':
						if (resMap['alloy'].value > 75 && !game.science.get('nanotechnology').researched && resMap['science'].maxValue > 2e5 && workshop.get('rotaryKiln').researched && !resMap['unobtainium'].value) {stock += 150;}
						break;
				}
		...
		getLimRat: function (name, limited, limRat) {
			...
			switch(name) {
				...
				case 'slab': {
					...
					limRat = (resMap['scaffold'].value > 2500 && res.value > 2500 * (1 + solar)) ? 0.04 : limRat;
					break;
				}
				case 'ship': {
					...
					limRat = (game.workshop.get('geodesy').researched && !space.getProgram('moonMission').val && solar < 4) ? 0.7 : 0.4;
				...
				case 'plate': {
					...
					let Temple = temple.on && temple.on < 15 - 3 * renaissance && solar && res.value < templeFactor * 15;
				...
				case 'alloy': {
					...
					let Reactor = !reactor.val && magneto.val > 31;
				...
				case 'concrate': {
					...
					let embassy = game.diplomacy.get('nagas').embassyLevel > 10;
					limRat = (reactor.val && embassy) ? 0.6 : 0.15;
					limRat = (res.value > Math.max(moon, 50 * factor, 10 * dataFactor)) ? 0.01 : limRat;
					limRat = (!res.unlocked && embassy && !game.calendar.season && resMap['titanium'].value > 500) ? 1 : limRat;
					break;
				}
				case 'gear': {
					...
					let fuelInjectors = this.getUnResearched('fuelInjectors') && resPercent('oil') < 0.8 && resPercent('coal') < 0.5 && res.value < 250 && resMap['titanium'].value > 200;
				...
				case 'kerosene':
					...
					break;
				case 'manuscript': {
					limRat = (game.prestige.getPerk("numeromancy").researched && !game.calendar.festivalDays && game.science.get("drama").unlocked) ? 0.25 : 0.47;
					limRat = (reactor.val) ? 0.4 : limRat;
				...
				case 'scaffold': {
					...
					let logistics = res.value < 1e3 && !game.getEffect('skillMultiplier') && game.village.leader && resMap['science'].maxValue > 1e5;
					limRat = (craftObservatory || resMap['beam'].value > 20 * resMap['slab'].value || logistics) ? 0.75 : limRat;
					limRat = (game.science.get('navigation').unlocked && resMap['iron'].value > 750 || factor > 2) ? limRat : 0;
					limRat = (val < 100 + 100 * solar && game.science.get('navigation').unlocked && resMap['iron'].value > 750) ? 1 : limRat;
					break;
				}
				case 'tanker':
					limRat = 0.1;
	...
	BulkManager.prototype = {
			...
			while (countList.length !== 0) {
				...
				for (let j = 0; j < countList.length; j++) {
					...
					for (let p = 0; p < prices.length; p++) {
						...
						if (source === 'space' && priceRes.name === 'oil') {
							spaceOil = true;
							oilPrice = priceRes.val * (1 - game.getLimitedDR(game.getEffect('oilReductionRatio'), 0.75));
						} else if (build.id === 'cryochambers' && priceRes.name === 'karma') {
							cryoKarma = true;
						...
						if (spaceOil) {
							...
							nextPriceCheck = tempPool[priceRes.name] < price;
						}
						if (nextPriceCheck) {
							for (let p2 = 0; p2 < p; p2++) {
								if (source && source === 'space' && prices[p2].name === 'oil') {
									...
									let refundVal = prices[p2].val * Math.pow(priceRatio, valPlusLoopTime);
						...
						if (spaceOil) {
							...
							tempPool[priceRes.name] -= (priceRes.name === 'void') ? Math.ceil(pVal) : pVal;
							// 检查 NaN
							tempPool[priceRes.name] = tempPool[priceRes.name] ? tempPool[priceRes.name] : 0;
						}
					}
					build.count++;
				}
		...
		singleBuildPossible: function (data, prices, priceRatio, source) {
			...
			for (let price in prices) {
				...
				let resPriceDiscount = game.getLimitedDR(game.getEffect(Res.name + "CostReduction"), 1);
				let resPriceModifier = 1 - resPriceDiscount;
				let rightPrice = Res.val * priceModifier * resPriceModifier;
				if (source === 'space' && Res.name === 'oil') {
				...
				} else if (data.name === 'cryochambers' && Res.name === 'karma') {
					...
					if (this.craftManager.getValueAvailable(Res.name, true) < rightPrice * Math.pow(priceRatio, data.val)) {return false;}
				}
			}
			return true;
		}
	...
	TradeManager.prototype = {
		...
		getProfitability: function (name) {
			...
			if (name === 'nagas') {
				...
				if (Val < 100 * production && race.embassyLevel > 10 && Val && (resMap['titanium'].value > 30 * production || (spice && game.calendar.festivalDays) || Val < 50)) {doTrade = true;}
				if (Val > 250 * (1 + solar) * (1 + production) && titaniumTri < 0.8) {prof = false;}
			}
			...
			if (name === 'griffins') {
				...
				if (titanium.value < 500 && season === 2 && solar > 2 && solar) {prof = false;}
				if (game.challenges.isActive("blackSky") && titanium.value < 500 && game.religion.getRU("basilica").on && season === 2) {doTrade = true;}
				// 缺铁贸易狮鹫
				if (resMap['steel'].value > 5 * resMap['plate'].value && titanium.value > 500 && season === 2) {doTrade = true;}
			...
			if (name === 'zebras') {
				...
				let glass = resMap['gold'].value > 200 || (doTrade && resMap['faith'].value > 450);
				if (glass && !game.religion.getRU("stainedGlass").on && solar) {return;}
				if (game.religion.getRU("basilica").on) {
					if (titanium.value < 30 || options.auto.cache.resUpg['alloy'] < 1e3) {
						doTrade = true;
					}
				} else {
				...
				if (season === 1) {
					if (game.workshop.get("geodesy").researched && (!game.workshop.get("concreteHuts").researched || resPercent('titanium') < 0.5)) {doTrade = true;}
					if (resMap['ship'].value > 240) {
						if (resMap['steel'].value > resMap['plate'].value || titaniumTri < 0.5) {doTrade = true;}
					}
				}
				if (!game.workshop.get('caravanserai').researched && solar < 4 && output['titanium'] < 1) {return false;}
				doTrade = (doTrade || spice && game.calendar.festivalDays);
			}
			if (name === 'spiders') {
				if (season === 2) {
