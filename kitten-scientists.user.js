		...
		upgrade: function () {
					...
					if (upgrades.techs.limited) {
						...
						switch (name) {
							...
							case 'oilProcessing':
								...
								break;
							case 'drama':
								if (craftManager.getTickVal(resMap['parchment'], true) < 4 && resMap['parchment'].value < 1e4) {
									msgSummary('drama');
									continue;
								}
								break;
			...
			if (upgrades.upgrades.enabled && game["workshopTab"].visible) {
				...
				if (Limited) {
					...
					// 没测地学过滤 钛金锯
					if (revolutionRatio < 6 && !geodesy && resStarchart.value > 400) {noop.push('titaniumSaw');}
					// 冷藏
					if (!geodesy) {
