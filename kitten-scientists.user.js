...
var run = function() {
	...
	var i18nData = {
		...
		'zh': {
			...
			'summary.auto.biolab': '为了节省合金发展，轨道测地学前不建造，太空制造前生物实验室优先级降低',
			'summary.auto.broadcastTower': '为了节省钛发展，太空制造解锁后建造更多的广播塔',
			'summary.auto.hunter': '未发明弩，故降低猎人的上限数量',
	...
	Engine.prototype = {
		...
		worship: function () {
			...
			if (option.bestUnicornBuilding.enabled) {
				...
				if (!tears && unicorns >= 2500 && zigguratOn) {
					resMap['tears'].value += zigguratOn;
					resMap['unicorns'].value -= 2500;
				}
				if (btn) {
					if (btn.opts) {
			...
			if (Math.min(0.999, Math.max(0.98, PraiseSubTrigger)) <= rate || doAdoreAfterTimeSkip) {
				...
				let expectSolarRevolutionRatio = Math.min(0.7 * Math.pow(Math.E, 0.66 * transformTier), solarLimmit);
				...
				booleanForAdore = (booleanForAdore && worship >= 1e5 && moonBoolean && game.religion.getSolarRevolutionRatio() > expectSolarRevolutionRatio * 0.01);
		...
		upgrade: function () {
			...
			if (upgrades.techs.enabled && game.libraryTab.visible) {
				...
				for (let upg of tech) {
					...
					for (var resource of prices) {
						...
						if (upgradeManager.getValue(resource.name) < resource.val) {
							continue techLoop;
						}
					}
					refreshRequired = 1;
			...
			if (upgrades.upgrades.enabled && game.bld.getBuildingExt('workshop').meta.on) {
				...
				if (upgrades.upgrades.limited) {
					...
					// 过滤钛升级
					if (game.globalEffectsCached['titaniumPerTickAutoprod'] < 0.002 && resMap['ship'].value < 50){
						noup = noup.concat(['titaniumAxe']);
					}
					//过滤测地学
					if (game.religion.getSolarRevolutionRatio() > 9 && !game.space.meta[0].meta[3].val && resMap['starchart'].value < 1600) {
						noup = noup.concat(['geodesy']);
					}
					// 没测地学过滤 地外计划
					if (!game.workshop.get('geodesy').researched) {
						noup = noup.concat(['seti']);
					}
					// 微型亚空间
					...
					} else if (game.challenges.isActive('blackSky') && resMap['starchart'].perTickCached < 1) {
						noup = noup.concat(['geodesy']);
					}
					//钍反应堆
				...
				(function (){
					...
					for (var i of toResearch) {
						for (var resource of i.prices) {
							if (craftManager.getValueAvailable(resource.name, true) < resource.val * 2) {
								continue;
							}
			...
			if (upgrades.buildings.enabled) {
				...
				if (pastureMeta.stage === 0 && options.auto.build.items.solarFarm.enabled) {
					if (pastureMeta.stages[1].stageUnlocked) {
						var energy = (winterProd && winterProd - 45 < game.resPool.energyCons);
		...
		build: function (builds) {
			...
			if (typeof builds != 'object') {
				...
				let scienceBuild = (name, max, trigger) => {
					...
					if (name) {
						if (scienceTrigger < trigger) {
							...
							bld.max = game.bld.getBuildingExt(name).meta.val + 2;
						}
					}
				};
				...
				if (scienceTrigger < 0.98 && !astronomy.researched && !machinery && astronomy.unlocked)  {
					items['academy'].enabled = false;
				}
				//图书馆牧场
				if (!theology) {
					...
					items['library'].max = (hutVal < 0) ? items['library'].max : Math.floor(hutVal * 11);
				}
				...
				if (!solarMeta.on && !atheism && theology) {
					if (options.auto.faith.items.solarRevolution.enabled && faithMeta.maxValue > 750 && resMap['gold'].maxValue > 500) {
					...
					tradepost.max = 12;
					if (goldTri > 0.96) {
						temple.max = game.bld.getBuildingExt('tradepost').meta.val + 1;
					}
					msgSummary('tradepost');
				} else if (theology) {
				...
				if (!orbitalGeodesy) {
					if (scienceMap.value > 150000 && resMap['oil'].maxValue > 35000) {
						calciner.max = (calcinerMax === -1) ? 40 : Math.min(40, calcinerMax);
					} else {
						let a = (1 + game.prestige.getParagonProductionRatio() * 10) * (1 + game.religion.getSolarRevolutionRatio());
						calciner.max = (calcinerMax === -1) ? a : Math.min(a, calcinerMax);
					}
				} else if (!spaceManufacturing || !game.space.meta[0].meta[0].on) {
					calciner.max = (calcinerMax === -1) ? 80 : Math.min(80, calcinerMax);
				}
				...
				if (game.science.get('electronics').researched && !spaceManufacturing && renaissance) {
				...
				if (!geodesy && !orbitalGeodesy) {
					...
					if (titaniumMore && !mansion.max && titaniumMap.value / titaniumMap.maxValue == 1) {
						mansion.max = 45;
						msgSummary('mansion');
					}
		...
		space: function () {
			...
			} else if (trigger === 0 && solarRevolution > 7.5 && !game.space.meta[0].meta[3].val && resMap['starchart'].value < 2000) {
				trigger = 1.1;
			}

			if (blackSky || solarRevolution > 7.5) {
				if (game.space.getBuilding('sattelite').val < 4 && builds.sattelite.enabled && buildManager.getBuildButton("sattelite")) {
		...
		hunt: function () {
			...
			let preCount = (!resMap['furs'].value || resMap['parchment'].value < 25) ? 30 : 0;
		...
		trade: function () {
			...
			for (var name in optionTrade.items) {
				...
				miningDrillMoon &= !goldTrigger && (name != 'nagas' || race.embassyLevel > 9);
				miningDrillMoon &= !goldTrigger && (name != 'griffins' || race.embassyLevel > 9);
		...
		miscOptions: function () {
			...
			if (optionVals.buildEmbassies.enabled && !!game.diplomacy.races[0].embassyPrices) {
				...
				if (optionVals.buildEmbassies.subTrigger <= cultureTri) {
	...
	BuildManager.prototype = {
		...
		count: function(id, count) {
			...
			switch (id) {
				...
				case 'lumberMill':
					if (id == 'lumberMill') {
						if (game.bld.getBuildingExt(id).meta.val < 45) {
						...
						if (game.bld.getBuildingExt(id).meta.val > 100 && !spaceManufacturing) {
							halfCount = true;
						}
					}
					// falls through
	...
	CraftManager.prototype = {
		...
		getLowestCraftAmount: function (name, limited, limRat, aboveTrigger) {
			...
			if (name === 'ship') {
				let optionVal = options.auto.options.enabled && options.auto.options.items.shipOverride.enabled;
				...
				force = (optionVal && shipValue < optionShipVal);
			}
			...
			if (name === 'plate' && limited && options.auto.craft.items['steel'].enabled && resMap['faith'].maxValue > 750) {
				let coalTick = game.getResourcePerTick('coal', true);
				let coalTri = this.getResource('coal').maxValue * trigger;
				if (coalTick > 0 && resMap['iron'].value != resMap['iron'].maxValue) {
				...
				//let platePrice = 15 * priceRatio;
				//if (resMap[name].value > platePrice && !gScience.get('navigation').researched) {
				//	autoMax = 0;
				//}
				if (!shipValue && ratio > 4 && resMap['starchart'].value >= 25) {
			...
			if (name === 'manuscript' && limited) {
				...
				for (i = 16; i < indexMax; i++) {
					...
					if (!meta.researched || cacheManuscript > 0) {
						...
						if (resVal > autoMax * craftPrices && autoMax >= 1 && resMap['culture'].value > craftPrices * 16 * autoMax) {
							...
							cache.science = (resMap['faith'].maxValue > 750 || resMap['gold'].value < 50) ? cache.science : "神殿";
				...
				if (!scienceMeta.meta[17].researched && aboveTrigger) {
					force = true;
				}
			}

			let religion = game.religion.getRU('solarRevolution').on && game.religion.faith > 1000;
				...
				for (i = 18; i < indexMax; i++) {
					...
					if (!meta.researched || cacheCompedium > 0) {
						...
						if (meta.prices[1].name == name || cacheCompedium) {
							...
							let scienceCheck = scienceVal > 1e4 * autoMax || resMap['science'].value >= resMap['science'].maxValue;
				...
				for (i = 30; i < 44; i++) {
					...
					if (!meta.researched) {
						...
						if (meta.prices[1].name == name) {
							...
							let scienceCheck = scienceVal > 2.5e4 * autoMax || resMap['science'].value >= resMap['science'].maxValue;
			...
			if (name === 'steel' && limited && options.auto.craft.items['plate'].enabled) {
				...
				let forceSteel = (name, prices) => {
					...
					if (!name || !workshopMeta.researched) {
						...
						if (name === 'oxidation' && amt > 0) {
							if (amt / Math.min(resMap['iron'].perTickCached, resMap['coal'].perTickCached) < 15) {
				...
				if (calciner.val) {
					...
					if (calciner.val > 1) {
						...
						if (!game.workshop.get('orbitalGeodesy').researched && resMap['oil'].value > 1.7e4 && resMap['oil'].maxValue > 3.5e4) {
							let a = Math.ceil((1000 - resMap['alloy'].value) / ratio) * 75;
							if (a > 0 && resMap['titanium'].value > a * 10 && !cacheUpg.cache) {
								options.auto.cache.resUpg['steel'] = a * 75;
						...
						if (flu.unlocked && !flu.researched) {
							let a = Math.ceil((200 - resMap['alloy'].value) / ratio);
							if (a > 0 && resMap['titanium'].value > a * 10 && !cacheUpg.cache) {
								options.auto.cache.resUpg['steel'] = a * 75;
						...
						if (options.auto.cache.resUpg['steel']) {
							forceSteel('', [{name:'steel',val:options.auto.cache.resUpg['steel']}]);
						}
					}
		...
		getStock: function (name) {
			...
			if (name === 'iron') {
				let reinforcedSaw = unResearched('reinforcedSaw') && resMap[name].value > 400 && resMap[name].maxValue > 1000;
				if (reinforcedSaw) {
					stock += 1000;
				}
				let crossbow = unResearched('crossbow') && game.bld.get('lumberMill').val > 33;
				if (crossbow) {
					stock += 1500;
				}
				let ironwood = unResearched('ironwood') && resMap[name].value > 1000 && resMap[name].maxValue > 3000;
				if (ironwood) {
					stock += 3000;
				}
			}
			if (name === 'steel') {
				let steelAxe = unResearched('steelAxe') && resMap[name].value > 38;
				if (steelAxe) {
					stock += 75;
				}
		...
		getLimRat: function (name, limited, limRat) {
			...
			if (name === 'beam' && limited) {
				limRat = (resMap['scaffold'].value || res.maxValue < 4000) ? limRat : 0.01;
			}
			if (name === 'slab' && limited) {
				let a = resMap['faith'].maxValue < 750 && res.value < 51;
				limRat = (resMap['scaffold'].value || res.value < 51) ? limRat : 0.01;
			}

			if (name === 'ship' && limited) {
				let shipLimit = 5 * game.bld.get("reactor").on + 225;
				let titaniumMax = resMap['titanium'].maxValue;
				let space = (!game.space.getBuilding('researchVessel').val && game.space.getBuilding('sattelite').val < 9);
				space = space && game.religion.getSolarRevolutionRatio() > 7.5;
			...
			if (name === 'plate' && limited) {
				...
				limRat = (game.getEffect('calcinerRatio')) ? limRat : 0.20;
				limRat = (resMap['faith'].maxValue < 750 && resMap['gold'].value > 40 && res.value < 30.5) ? 1 : limRat;
				limRat = (res.value < 150 && game.science.get('navigation').researched || ractor) ? 0.75 : limRat;
			}

			if (name === 'alloy' && limited) {
				let titaniumTick = game.globalEffectsCached['titaniumPerTickAutoprod'] == 5e-4;
				limRat = (game.bld.get("steamworks").on < game.bld.get("magneto").on || titaniumTick) ? limRat : 0.75;
